...
# CCF of External TP Load vs Deep MSFD Indicators
# Figure 2 for hysteresis article
# ------------------------------------------------------------------------------
...
for (var in target_vars) {
  ...
  ccf_out <- ccf(grand_df$TP_load, grand_df[[var]], plot = FALSE, lag.max = 25)
...
p_ccf <- ggplot(ccf_results, aes(x = Lag, y = Correlation)) +
  ...
  labs(
    title = "Causally Constrained CCF: TP Load vs Deep Indicators",
...
ggsave(
  filename = "Figure_2_Baltic_CCF_TP_HELCOM_deep.png", 
  plot = p_ccf, 
  width = 10,      
...
print("Analysis complete. Figure 2 successfully saved.")
